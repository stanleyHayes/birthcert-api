...
exports.register = async (req, res) => {
    try {
        ...
        const url = `https://birthregistry.vercel.app/auth/verify/${token}`;
        const message = `Click on the link ${url} and verify your email with the otp ${otp}`;
        await admin.save();
        await sendEmail(email, 'VERIFY ACCOUNT', message);
...
exports.forgotPassword = async (req, res) => {
    try {
        ...
        admin.authInfo.token = token;
        const url = `https://birthregistry.vercel.app/auth/reset-password/${token}`;
...
exports.updateProfile = async (req, res) => {
    try {
        ...
        const allowedUpdates = ['name', 'email', 'phone', 'status', 'username'];
